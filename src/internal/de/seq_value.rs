...
impl<'t, 'de> SeqAccess<'t, 'de> {
    fn new(len: Option<usize>, element: TypeId, defs: &'t Types, msg: &'t mut Message<Cursor<&'de [u8]>>) -> Result<SeqAccess<'t, 'de>, Error> {
        let remaining_count = msg.read_uint()?;

        if let Some(len) = len {
            if remaining_count != len as u64 {
                return Err(de::Error::custom(format!("sequence len mismatch (expected {}, got {})", len, remaining_count)));
            }
        }
...
pub(crate) struct SeqValueDeserializer<'t, 'de> where 'de: 't {
    len: Option<usize>,
    element: TypeId,
    defs: &'t Types,
    msg: &'t mut Message<Cursor<&'de [u8]>>
...
impl<'t, 'de> SeqValueDeserializer<'t, 'de> {
    #[inline]
    pub(crate) fn new(len: Option<usize>, element: TypeId, defs: &'t Types, msg: &'t mut Message<Cursor<&'de [u8]>>) -> SeqValueDeserializer<'t, 'de> {
        SeqValueDeserializer { len, element, defs, msg }
    }
}

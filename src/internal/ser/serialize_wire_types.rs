...
use super::{SerializationCtx, FieldValueSerializer};
...
impl<'a> SerializeWireTypes<'a> {
    pub fn new(wire_types: &'a mut Vec<(TypeId, Vec<u8>)>) -> Self {
        SerializeWireTypes { len_pre: wire_types.len(), wire_types }
    }
    ...
    fn serialize_main_type(&mut self, id: TypeId, ty: &Type<TypeId>) -> Result<(), Error> {
        ...
        let ok = match ty {
            ...
            &Type::Seq { len, element } => {
                if let Some(len) = len {
                    ser.serialize_newtype_variant("WireType", 0, "ArrayT",
                        &SerializeArrayType { id, len: len as i64, elem: element })?
                } else {
                    ser.serialize_newtype_variant("WireType", 1, "SliceT",
                        &SerializeSliceType { id, elem: element })?
                }
            },
            &Type::Map { key, value } => {
                ser.serialize_newtype_variant("WireType", 3, "MapT",
                    &SerializeMapType { id, key: key, elem: value })?
            },
    ...
    fn serialize_enum_variants(&mut self, mut next_id: TypeId, ty: &Type<TypeId>) -> Result<(), Error> {
        if let &Type::Enum { ref variants, .. } = ty {
            for variant in variants.iter() {
                if let &EnumVariant::Struct { ref name, ref fields } = variant {
                    let ctx = SerializationCtx::new();
                    let ok = {
                        let mut ser = FieldValueSerializer { ctx, type_id: TypeId::WIRE_TYPE };
                        ser.serialize_newtype_variant("WireType", 2, "StructT",
                            &SerializeStructType { id: next_id, name, fields })?
                    };
...
impl<'a> Serialize for SerializeEnumStructType<'a> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        s.serialize_field("common", &SerializeCommonType { id: self.id, name: self.name })?;
        s.serialize_field("Fields", &SerializeEnumStructFields { id: self.id, variants: self.variants })?;
        s.end()
    }
}
...
impl Serialize for SerializeMapType {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        s.serialize_field("common", &SerializeCommonType { id: self.id, name: "" })?;
...
impl<'a> Serialize for SerializeStructType<'a> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut s = serializer.serialize_struct("StructType", 2)?;
        s.serialize_field("common", &SerializeCommonType { id: self.id, name: self.name })?;
        s.serialize_field("Fields", &SerializeStructFields { fields: self.fields })?;
        s.end()
    }
}
...
impl<'a> Serialize for SerializeStructFields<'a> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        for field in self.fields {
            s.serialize_element(&SerializeStructField { name: &field.name, id: field.id })?;
        }
        s.end()
    }
}
